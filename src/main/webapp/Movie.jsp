...
<div class="main">
    ...
    <div class = "addMovie_main">
        <div class="add_movie_frame">
            <h1 class="add_movie_title">修改电影</h1>
            <form action="UpdateMovieServlet" method="post">
                <input type="hidden" name="id" value="${movie.id}">
                <input placeholder="请输入电影名" class="addMovieInput" type="text" name="movieName" value="${movie.movieName}" required="required"/>
                </br>
                <input placeholder="请输入电影评分 格式:X.X" class="addMovieInput" type="text" name="movieGrade" value="${movie.movieGrade}" required="required" min="0" max="10" onkeyup="value=value.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')"/>
                </br>
                <input placeholder="上映日期 格式:XXXX-XX-XX" class="addMovieInput" type="text" name="date" value="${movie.date}" required="required"/>
                </br>
                <textarea placeholder="请输入描述" class="descriptionInput" type="text" name="description" required="required">${movie.description}</textarea>
                </br>
